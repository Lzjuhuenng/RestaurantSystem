...
<%	
	...
	Page<Account> accPage =(Page<Account>) request.getAttribute("accPage"); 
	IAccountTypeService accTypeService = new AccountTypeServiceImpl();
	Account acc = accPage.getResultList().get(accPage.getResultList().size()-1);
	accPage.getResultList().remove(accPage.getResultList().size()-1);
	...
	if(request.getParameter("account")!=null&&!"".equals(request.getParameter("account"))){
		urlParam += "&account="+request.getParameter("account");
	}
	if(request.getParameter("username")!=null&&!"".equals(request.getParameter("username"))){
		urlParam += "&username="+request.getParameter("username");
	}
	if(request.getParameter("role")!=null&&!"".equals(request.getParameter("role"))){
		urlParam += "&role="+request.getParameter("role");
	}
...
<nav style="height: 450px;">
	<ul>
		...
		<li>
			...
			<ul class="submenu">
				<li><a href="/admin/dmin_add_food.jsp">添加菜品信息</a></li>
...
<section class="content" style="margin-top: 10px;">
	<section class="widget" style="height: 630px">
		...
		<div class="content" style="padding-top: 5px;">
			<form method="get" action="/servlet/AccountOperateServlet">
			...
			<table id="myTable" border="0" width="100">
				<thead style="text-align: center;">
					...
					int index = 1;
					for(Account acco : accPage.getResultList()){
					...
					<tr>
							...
							<td><a href="/servlet/AccountOperateServlet?action=update&id=<%=acco.getId()%>"">修改</a></td>
							<td><a href="/servlet/AccountOperateServlet?action=del&id=<%=acco.getId()%>">删除</a></td>
						</tr>
					<%	
					}
		...
		<div class="text-center" style="position:absolute;bottom:-230px ;padding-left: 270px;padding-right: 270px;">
			<ul class="pagination">
				
				<li <%=accPage.getPageNo()==0?"class='disabled'":"" %> ><a href="/servlet/AccountOperateServlet?action=query<%=urlParam %>&pageNo=0">首页</a></li>
				<li <%=accPage.getPageNo()==0?"class='disabled'":"" %> ><a href="/servlet/AccountOperateServlet?action=query<%=urlParam %>&pageNo=<%=accPage.getPageNo()-1 %>">上一页</a></li>
				<% 
					for(int i = accPage.getPageNo()-2,j=0;j < 6 &&i<=accPage.getTotalPageNo();i++,j++ ){
						if(i<0 ){
							continue;
						}
				
				%>
				<li <%=accPage.getPageNo()==i?"class='active'":"" %> ><a href='/servlet/AccountOperateServlet?action=query<%=urlParam %>&pageNo=<%=i %>'><%=i+1 %></a></li>
				
				<% 
				}
				%>
				<li <%=accPage.getTotalPageNo()==accPage.getPageNo()?"class='disabled'":"" %> ><a href="/servlet/AccountOperateServlet?action=query<%=urlParam %>&pageNo=<%=accPage.getPageNo()+1 %>">下一页</a></li>
				<li <%=accPage.getTotalPageNo()==accPage.getPageNo()?"class='disabled'":"" %> ><a href="/servlet/AccountOperateServlet?action=query<%=urlParam %>&pageNo=<%=accPage.getTotalPageNo() %>">尾页</a></li>
			</ul>
		</div>
		</div>
	</section>
